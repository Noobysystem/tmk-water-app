...
with tab6:
    ...
    with st.expander("📏 1. Геометрия резервуаров и расчет остатка по рулетке (в метрах)", expanded=True):
        st.markdown("""
        #### 1. Резервуар расходный в машинном зале ($2.8 \\times 2.8 \\times 2.5 \\text{ м}$)
        * **Площадь зеркала:** $2.8 \\times 2.8 = 7.84 \\text{ м}^2$.
        * **Полная вместимость:** $19\\,600 \\text{ литров}$ ($19.60 \\text{ м}^3$).
        * **Формула остатка:** $V = 2.8 \\times 2.8 \\times (2.50 - h_{\\text{замера}}) \\times 1000 \\text{ (л)}$.

        #### 2. Резервуар №1 на складе мокрохранения ($5.8 \\times 5.8 \\text{ м}$, высота $2.5 \\dots 2.8 \\text{ м}$)
        * **Перепад высоты дна (уклон):** $\\Delta H = 2.8 - 2.5 = 0.30 \\text{ м}$ ($30 \\text{ см}$).
        * **Объем нижнего клина:** $V_{\\text{клин}} = 0.5 \\times 5.8 \\times 5.8 \\times 0.30 \\times 1000 = 5\\,046 \\text{ литров}$.
        * **Полная вместимость:** $89\\,146 \\text{ литров}$ ($89.15 \\text{ м}^3$).

        #### 3. Резервуар №4 на складе мокрохранения ($2.8 \\times 5.8 \\text{ м}$, высота $2.7 \\dots 3.4 \\text{ м}$)
        * **Перепад высоты дна (уклон):** $\\Delta H = 3.4 - 2.7 = 0.70 \\text{ м}$ ($70 \\text{ см}$).
        * **Объем нижнего клина:** $V_{\\text{клин}} = 0.5 \\times 2.8 \\times 5.8 \\times 0.70 \\times 1000 = 5\\,684 \\text{ литра}$.
        * **Полная вместимость:** $49\\,532 \\text{ литра}$ ($49.53 \\text{ м}^3$).
        """)
