...
# =============================================================================
# ВКЛАДКА 6: ПАМЯТКА И РАСЧЕТЫ (ПОДРОБНЫЙ СПРАВОЧНИК)
# =============================================================================
with tab6:
    ...
    with st.expander("🧪 1. Подробно про Коагулянт, Флокулянт и соотношение 22:1", expanded=True):
        st.markdown("""
        #### Физико-химический механизм действия реагентов
        1. **Коагулянт («Эпоха» / Полиоксихлорид алюминия $\\text{Al}^{3+}$):**
           * **Задачи:** Ионы тривалентного алюминия нейтрализуют отрицательный дзета-потенциал коллоидных частиц грязи и взвеси.
           * **Результат:** Частицы теряют взаимное отталкивание и объединяются в микрохлопья size $0.05 \\dots 0.1 \\text{ мм}$.
           * **Типичные рабочие дозы:** $10 \\dots 18 \\text{ мг/л}$.

        2. **Флокулянт («ЭкоПлюс» / Полиакриламид ПАА):**
           * **Задачи:** Длинные полимерные цепочки адсорбируются на поверхности нескольких микрохлопьев одновременно (эффект «мостообразования»).
           * **Результат:** Формируются крупные, тяжелые макрохлопья size $1 \\dots 3 \\text{ мм}$, быстро осаждающиеся в осветлителях.
           * **Типичные рабочие дозы:** $0.3 \\dots 0.75 \\text{ мг/л}$.

        #### Защитный предел фильтрации (СП 31.13330.2012 / СНиП 2.04.02-84)
        * Доза флокулянта строго ограничена значением **$0.75 \\text{ мг/л}$**.
        * **Причина:** Превышение нормы приводит к созданию высоковязкой полимерной пленки на кварцевом песке, забиванию пор фильтра («залепливанию») и срыву фильтроцикла.

        #### Почему в программе выбрано базовое соотношение 22:1?
        * **Соотношение $22:1$ не является жестким физическим законом.** Это расчетно-защитный коэффициент, выведенный из рабочих диапазонов.
        * При верхней рабочей дозе коагулянта $16.5 \\text{ мг/л}$ и максимально допустимой дозе флокулянта $0.75 \\text{ мг/л}$:
          $$\\frac{16.5 \\text{ мг/л}}{0.75 \\text{ мг/л}} = 22$$
        * Данное соотношение гарантирует, что при увеличении дозировки коагулянта автоматика не передозирует флокулянт выше $0.75 \\text{ мг/л}$.

        #### Исследования ВНИИ ВОДГЕО и экспериментальный подбор (Jar Test)
        * Согласно исследованиям ВНИИ ВОДГЕО, оптимальная пропорция варьируется от **$15:1$ до $30:1$** и зависит от:
          1. **Температуры воды** (в зимний период при $T < 4^\\circ\\text{C}$ гидратация и формирование хлопьев замедляются);
          2. **Сезонности** (в период паводка резко меняется цветность и органика);
          3. **$\\text{pH}$ и щелочности среды**.
        * **Практическое правило:** Точная пропорция определяется лабораторией цеха методом **пробного коагулирования в цилиндрах (Jar Test)**, после чего коэффициент $1:N$ корректируется в соответствующем поле на 1-й вкладке.
        """)
    ...
    with st.expander("🛢 2. Затворение раствора коагулянта по рулетке"):
        ...
        * **Плотность товарного концентрата:** **$1.24 \\text{ г/см}^3$**.
        ...
        #### Формулы расчета объемов долива
