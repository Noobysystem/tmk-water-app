...
with tab1:
    ...
    with st.expander("📏 Затворение коагулянта по рулетке (Расходный бак 2.8х2.8х2.4 м)", expanded=True):
        st.caption("Машинист замеряет расстояние от верхнего края бака до зеркала воды перед приготовлением смены.")
        
        prep_col1, prep_col2 = st.columns(2)
        
        with prep_col1:
            h_measured_cm = st.number_input(
                "Расстояние от края бака до воды (по рулетке), см", 
                value=100.0, 
                step=5.0, 
                min_value=0.0, 
                max_value=240.0,
            ...
            C_target_prep = st.number_input(
                "Целевая концентрация рабочего раствора, %", 
                value=1.0, 
                step=0.1, 
                key="c_prep"
            )
        ...
        V_TOTAL_TANK = AREA_TANK * 2.4 * 1000.0  # 18816 литров при 2.4 м
        ...
        if V_add_l > 0 and C_tov > 0:
            rho_work = 1.010
            M_dry_req = (V_add_l / 1000.0) * 1000.0 * (C_target_prep / 100.0) * rho_work
            M_tov_req = M_dry_req / (C_tov / 100.0)
            V_tov_req_l = M_tov_req / rho_tov
            V_water_add_l = V_add_l - V_tov_req_l

            st.markdown("### 📊 Результат расчета для машиниста:")
            
            m_col1, m_col2, m_col3 = st.columns(3)
            m_col1.metric("Остаток в баке", f"{V_remain_l:.0f} л", help=f"Глубина остатка: {240 - h_measured_cm:.0f} см")
            m_col2.metric("Общий объем долива", f"{V_add_l:.0f} л")
            m_col3.metric("КОНЦЕНТРАТ из Еврокуба", f"{V_tov_req_l:.1f} л", delta=f"{M_tov_req:.1f} кг")

            st.info(f"💦 **Объем технической воды для долива:** **{V_water_add_l:.0f} литров**")

            st.markdown("#### 📝 Инструкция по шагам:")
            st.markdown(f"""
            1. Отмерить по риске еврокуба и залить в расходный бак **{V_tov_req_l:.0f} литров** (или **{M_tov_req:.0f} кг**) концентрата «Эпоха».
            2. Долить техническую воду до верхнего края бака (долить **{V_water_add_l:.0f} л** воды).
            3. Включить **барботаж** и перемешивать раствор не менее 15–20 минут перед подачей на насосы.
            """)
