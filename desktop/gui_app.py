...
------------------------------------------------------------------------------
1. МЕХАНИЗМЫ РЕАГЕНТНОЙ ОБРАБОТКИ И СООТНОШЕНИЕ 22:1
------------------------------------------------------------------------------
• Коагулянт «Эпоха» (Полиоксихлорид алюминия Al³+):
  - Задача: Ионы Al³+ нейтрализуют отрицательный дзета-потенциал загрязнений.
  - Результат: Микрохлопья размером 0.05–0.1 мм.
  - Рабочие дозы: 10–18 мг/л.

• Флокулянт «ЭкоПлюс» (Полиакриламид ПАА):
  - Задача: Полимерные цепи связывают микрохлопья между собой («мостообразование»).
  - Результат: Крупные тяжелые хлопья размером 1–3 мм.
  - Рабочие дозы: 0.3–0.75 мг/л.

• Предел фильтрации (СП 31.13330.2012):
  Доза флокулянта ограничена верхним пределом 0.75 мг/л. Избыток создаёт
  вязкую полимерную плёнку на кварцевом песке и забивает пор фильтра.

• Почему выбрано базовое соотношение 22:1?
  Соотношение 22:1 является расчетным защитным коэффициентом:
  При верхней рабочей дозе коагулянта 16.5 мг/л и пределе флокулянта 0.75 мг/л:
  16.5 / 0.75 = 22.
  Это предотвращает передозировку флокулянта при повышении дозы коагулянта.

• Исследования ВНИИ ВОДГЕО и пробное коагулирование (Jar Test):
  Оптимальная пропорция варьируется от 15:1 до 30:1 в зависимости от:
  1) Температуры воды (зимой при <4°C гидратация замедляется);
  2) Сезонности (паводок, цветность, органика);
  3) pH и щелочности среды.
  Точная доза подбирается лабораторией цеха методом пробного коагулирования,
  после чего пропорция (1:N) задаётся в поле на 1-й вкладке.

• Формула расхода насоса (л/ч):
  q = (Q * D) / (10 * C * rho)

------------------------------------------------------------------------------
2. ПРИГОТОВЛЕНИЕ РАСТВОРА В РАСХОДНОМ БАКЕ (2.8 x 2.8 x 2.4 м)
------------------------------------------------------------------------------
• Геометрия резервуара:
  - Площадь: 2.8 * 2.8 = 7.84 м²
  - Удельный объем: 1 см высоты = 78.4 литра
  - Полный объем при высоте 2.4 м = 18 816 литров

• Паспорт качества № 644 (АО «АУРАТ» от 06.08.2026):
  - Продукт: Полиоксихлорид алюминия «ЭПОХА» (марка 2)
  - Содержание Al3+: 9.2%
  - Плотность концентрата: 1.24 г/см³

• Расчет затворения по рулетке (h_замера, см):
  - Объем долива = h_замера * 78.4 л
  - Остаток = 18 816 - Объем долива
  - Масса сухого Al3+ = Объем долива * (C_цель / 100) * rho_раб
  - Концентрат из еврокуба (л) = (Масса сухого / 0.092) / 1.24
  - Вода для долева (л) = Объем долива - Концентрат

------------------------------------------------------------------------------
3. ИЗМЕРЕНИЕ МУТНОСТИ НА СПЕКТРОФОТОМЕТРЕ ПЭ-5300ВИ
------------------------------------------------------------------------------
• Методика ПНД Ф 14.1:2:4.213-05 (520 нм, кювета 50 мм).
• Формула расхода мутности C (ЕМ/дм³):
  C = (D - D0) / K
  Градуировочные константы: K = 0.009347066, D0 = 0.002417294

------------------------------------------------------------------------------
4. НАСТРОЙКА ЛИМБА НАСОСА «АРЕОПАГ»
------------------------------------------------------------------------------
• Шкала лимба: 0–60 делений (цена деления 0.5 мм).
• Оптимальная частота КИМ АДКФ: 35.0 Гц.
• Формула настройки: S_новое = round(S_тек * (f_тек / 35.0))
==============================================================================
"""
