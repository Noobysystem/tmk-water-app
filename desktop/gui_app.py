...
------------------------------------------------------------------------------
1. ДОЗИРОВАНИЕ РЕАГЕНТОВ (КИМ / УНИТОК)
------------------------------------------------------------------------------
• Целевая мутность осветлителей: 8.0 ЕМ/дм³
• Корректировка дозы коагулянта:
  Если М_осв > 8.0 ЕМ/дм³, доза коагулянта «Эпоха» увеличивается на 0.3 мг/л
  на каждые 2.0 ЕМ/дм³ превышения (минимальный шаг 0.5 мг/л, макс. 18.0 мг/л).

• Пропорция Коагулянт / Флокулянт:
  Задается в поле настройки (по умолчанию 1:22).
  Доза флокулянта «ЭкоПлюс» = (Доза коагулянта / N).
  Защитный предел: доза флокулянта <= 0.75 мг/л (СП 31.13330.2012).

• Формула расхода насоса (л/ч):
  q = (Q * D) / (10 * C * rho)
  где Q - расход воды (м³/ч), D - доза (мг/л), C - концентрация (%),
  rho - плотность рабочего раствора (г/см³).
  Число 10 в знаменателе - коэффициент математического перевода единиц.

------------------------------------------------------------------------------
2. ПРИГОТОВЛЕНИЕ РАСТВОРА В РАСХОДНОМ БАКЕ (2.8 x 2.8 x 2.4 м)
------------------------------------------------------------------------------
• Геометрия резервуара:
  - Площадь зеркала: 2.8 * 2.8 = 7.84 м²
...
• Расчет доления по рулетке h_замера (см):
  - Объем долива = h_замера * 78.4 л
  - Остаток в баке = 18 816 - Объем долива
...
------------------------------------------------------------------------------
• Методика ПНД Ф 14.1:2:4.213-05 (длина волны 520 нм, кювета 50 мм).
...
• Формула настройки: S_новое = S_тек * (f_тек / 35.0)
  (со сглаживанием и ограничением в диапазоне 0..60 делений).
==============================================================================
"""
